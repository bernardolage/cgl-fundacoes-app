/* ====================================================================
   ...
   - (até 21/09/2026 em_transito gerava remessa em movimentacoes_ativos e desmobilizar gerava retorno; o módulo
     Movimentações foi retirado do app e a geração automática foi desligada — criarMovimentacaoMob ficou sem uso)
...
async function mudarStatusMobilizacao(novo, btn){
  ...
  if(novo === "em_transito"){
    ...
    if(!confirm("Registrar saída para a obra? A TAG passa a 'em trânsito'.")) return;
  } else if(novo === "desmobilizada"){
    if(!confirm("Desmobilizar? A TAG volta para a base.")) return;
  ...
  try {
    ...
    // 21/09/2026: módulo Movimentações retirado do app — a mobilização não gera mais remessa/retorno em rascunho
    // (ninguém os completava: 0 registros). A localização da TAG segue vindo do status da mobilização.
